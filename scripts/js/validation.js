...
function checkLastName(){														//defines checkLastName function		
		...
		var lastNameVal = new RegExp(/^[A-Za-z]{3,20}$/);								//RegEx check for last name
		if(!lastName){
			...
			$('#lastNameControl').removeClass('success').addClass('error');				//add error class to control group (turns inline text red)
			$('#lastNameControl').find('.help-inline').html('Revise your entry to include at least 3  alphabetical characters (A-Z) only and resubmit.');//Feedback to user for resubmit 
		} else { 
...
function checkDiscussionPrompt(){		
		var pSize = $('#discussionPrompt').val().length; 
			 if (pSize > 1000) 
			 {
					...
					$('#discussionPromptControl').find('.help-inline').html('You have exceeded 1000 characters, please revise. You wrote a total of: <strong>' + pSize + '</strong> characters.');	 
			  }
			  else 
			  {
				  	$('#discussionPromptControl').removeClass('error');	
				  	$('#discussionPromptControl').find('.help-inline').html('If you like you can provide prompts to get into details or explain directions for the discussion. Please limit your text to 1000 characters. You wrote a total of: <strong>' + pSize + '</strong> characters.');	 
			  } 
}
